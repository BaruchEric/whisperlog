...
SAMPLE_DIR="$(mktemp -d -t whisperlog-demo-XXXXXX)"
...
echo "Transcribing..."
whisperlog transcribe "$SAMPLE_MP3"

echo
echo "Searching the archive for the word 'the'..."
whisperlog search "the" --limit 3 || true

echo
echo "Running local Ollama summarizer..."
whisperlog enrich "$SAMPLE_MP3" --backend ollama --task summarize || {
  echo "Ollama enrichment skipped (is 'ollama serve' running with qwen2.5:7b pulled?)"
}

echo
echo "Demo done. Archive at: $(whisperlog config show 2>/dev/null | grep whisperlog_archive_dir || true)"
